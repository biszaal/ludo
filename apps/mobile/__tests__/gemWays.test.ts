/**
 * Whether the Gems tab has any way in to offer.
 *
 * The tab offers two ways to get gems: packs and a rewarded ad. Either can
 * be switched off independently — `purchasesEnabled` is the billing flag, and
 * the ad row has its own kill switch (gemAdRow) — so the section has to survive
 * every combination without leaving a labelled heading above an empty row,
 * which is the specific ugliness this prevents.
 *
 * The rule: draw the heading when at least one way in is live, never over
 * nothing.
 */
...
describe("gemWaysView", () => {
  it("shows both ways in when both are live", () => {
    const v = gemWaysView({ tierOn: true, packsBuyable: true, adVisible: true });
  ...
  it("shows only the ad when packs cannot be bought", () => {
    ...
    const v = gemWaysView({ tierOn: true, packsBuyable: false, adVisible: true });
    expect(v).toEqual({ visible: true, showPacks: false, showAd: true });
  });

  it("shows only the packs when the ad placement is off", () => {
    const v = gemWaysView({ tierOn: true, packsBuyable: true, adVisible: false });
  ...
  it("draws nothing when neither way in is live", () => {
    ...
    const v = gemWaysView({ tierOn: true, packsBuyable: false, adVisible: false });
  ...
  it("draws nothing when the whole premium tier is off", () => {
    ...
    const v = gemWaysView({ tierOn: false, packsBuyable: true, adVisible: true });
    expect(v.visible).toBe(false);
