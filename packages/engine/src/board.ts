...
export function isSafeSquare(absoluteTrackIndex: number): boolean {
 ...
 */
export function toRelativeIndex(color: Color, position: TokenPosition): number | null {
  if (position === "home") return null;
  if (position === "finished") return FINISH_REL_INDEX;
  if (position.type === "homePath") return TRACK_PATH_LENGTH + position.index;
 ...
 */
export function fromRelativeIndex(color: Color, relIndex: number): TokenPosition {
  if (relIndex >= FINISH_REL_INDEX) return "finished";
 ...
 */
export function absoluteTrackIndex(position: TokenPosition): number | null {
