 ...
 */
export function validateMove(state: GameState, move: Pick<Move, "tokenId">): ValidationResult {
  ...
  const resolved = getValidMoves(state, state.currentTurnPlayerId).find(
    (m) => m.tokenId === move.tokenId,
  );
  if (!resolved) {
