 ...
 */
function stuckInYard(state, playerId) {
...
export function endTurn(state, options = {}) {
     ...
     */
    if (state.rules.threeRollsFromYard &&
        ...
        next.diceValue = null;
        next.lastAction = makeAction("endTurn", { playerId: state.currentTurnPlayerId, yardRoll: next.yardRolls }, options.now);
        return next;
    }
