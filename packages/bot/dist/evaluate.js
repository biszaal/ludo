 ...
 */
import { FINISH_REL_INDEX, TRACK_PATH_LENGTH, absoluteTrackIndex, toRelativeIndex, } from "@ludo/engine";
import { buildTrackIndex, captureProbAt, cellsAt, slotOf, threatProbAt } from "./threat.js";
/**
 ...
 */
export function tokenProgressValue(rel) {
    if (rel === null)
        return YARD_VALUE;
    if (rel >= FINISH_REL_INDEX)
        return FINISHED_VALUE;
    if (rel >= TRACK_PATH_LENGTH)
        return HOME_PATH_BASE + (rel - TRACK_PATH_LENGTH) * HOME_PATH_STEP;
 ...
 */
function allStrengths(state, risk, index) {
    ...
    for (let i = 0; i < n; i++)
        slots[i] = slotOf(index, state.players[i].id);
    for (const t of state.tokens) {
        ...
        if (owner < 0)
            continue; // a token with no seat — nothing to attribute it to
 ...
 */
export function riskAppetite(state, playerId) {
    let mine = 0;
    let best = 0;
    for (const p of state.players) {
        if (!inPlay(state, p) && p.id !== playerId)
            continue;
        let sum = 0;
        for (const t of state.tokens) {
            if (t.playerId !== p.id)
 ...
 */
export function evaluateFor(state, playerId, risk) {
